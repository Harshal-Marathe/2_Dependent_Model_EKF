...
def render_tab5(nevergrad_available: bool):
        ...
        with st.spinner(spinner_text):
            try:
                ...
                with st.expander("📊 R² metrics"):
                    ...
                    rc1.metric("R²",        f"{results_1['r2']:.4f}")
                    rc2.metric("Gelman R²", f"{results_1['r2_gelman']:.4f}")
                ...
                if results_2 is not None and chained_mode:
                    ...
                    with st.expander("📊 R² metrics"):
                        ...
                        rd1.metric("R²",        f"{results_2['r2']:.4f}")
                        rd2.metric("Gelman R²", f"{results_2['r2_gelman']:.4f}")
                    st.info(
    ...
    if st.session_state.model_fitted:
        ...
        with st.expander("📊 R² metrics"):
            rc1, rc2 = st.columns(2)
            rc1.metric("R²", f"{res['r2']:.4f}")
            rc2.metric("Gelman R²", f"{res['r2_gelman']:.4f}")
        ...
        if st.session_state.get("model_fitted_2") and st.session_state.get("model_results_2"):
            ...
            with st.expander("📊 R² metrics"):
                ...
                re1.metric("R²", f"{res2['r2']:.4f}")
                re2.metric("Gelman R²", f"{res2['r2_gelman']:.4f}")
